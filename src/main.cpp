...
uint256 hashGenesisBlock("0x00000c41035dee17d51e543c966cb506141c932feca229de8a6b66a0b824ce73");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BroadCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BroadCoin Signed Message:\n";
...
{
    // BroadCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // BroadCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block:
		...
		const char* pszTimestamp = "broad genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1504815900;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2256354;
...
unsigned char pchMessageStart[4] = { 0xf0, 0xc0, 0xb0, 0xde }; // BroadCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BroadCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BroadCoinMiner : ProcessBlock, block not accepted");
    }
